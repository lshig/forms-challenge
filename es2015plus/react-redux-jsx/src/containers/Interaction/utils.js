...
export const matchContentToTab = (contentId, previousFlag) => {
  for (var t = 0; t < tabContentMap.length; t++) {
...
export const setAttributeFromModel = (DOMNodeList, attributeName, modelData) => {
...
export const transformData = (modelData) => {
  ...
  for (let key in modelData) {
    if (key !== 'activeTabId' && key !== 'activeContentId' && key !== 'activeTime' &&
        key !== 'timerStatus' && key !== 'timeCounter' && key !== 'intervalId') {
